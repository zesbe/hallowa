-- Safest migration script - creates tables first, then drops and recreates dependent objects
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'whatsapp_conversations_device_id_contact_jid_key'
  ) THEN
    ALTER TABLE whatsapp_conversations ADD CONSTRAINT whatsapp_conversations_device_id_contact_jid_key UNIQUE(device_id, contact_jid);
  END IF;
END $$;
...
-- Now that tables exist, drop existing triggers if they exist
...
DROP INDEX IF EXISTS idx_conversations_user_device;
DROP INDEX IF EXISTS idx_conversations_updated_at;
DROP INDEX IF EXISTS idx_conversations_label;
...
DROP INDEX IF EXISTS idx_messages_device;
...
DROP POLICY IF EXISTS "Users can insert own conversations" ON whatsapp_conversations;
DROP POLICY IF EXISTS "Users can update own conversations" ON whatsapp_conversations;
DROP POLICY IF EXISTS "Users can delete own conversations" ON whatsapp_conversations;
DROP POLICY IF EXISTS "Service can manage all conversations" ON whatsapp_conversations;
...
DROP POLICY IF EXISTS "Users can update own messages" ON whatsapp_messages;
DROP POLICY IF EXISTS "Service can manage all messages" ON whatsapp_messages;
